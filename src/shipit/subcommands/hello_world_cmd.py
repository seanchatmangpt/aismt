...
def hello_world_command(
    ...
    asyncio.run(_hello_world_command(config))

async def _hello_world_command(config: OptionConfig, **kwargs):
    if config.verbose:
        typer.echo(f"Running hello_world command... {config}")
    ...
    if config.verbose:
        typer.echo(f"hello_world command finished.")


@app.command("test_123")
def test_123_command(
    prompt: str,
    model: str = typer.Option(
        "gpt-3.5-instruct",
        "-m",
    ...
    asyncio.run(_test_123_command(config))

async def _test_123_command(config: OptionConfig, **kwargs):
    if config.verbose:
        typer.echo(f"Running test_123 command... {config}")
    ...
    if config.verbose:
        typer.echo(f"test_123 command finished.")
