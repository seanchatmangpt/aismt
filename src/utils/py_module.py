...
import logging
import os
from ast import NodeTransformer, copy_location, fix_missing_locations, parse
        ...
        if self.red:
            for elem in new_element:
                ...
                if existing_element:
                    if "def" in elem.type:
                        self.replace_method(
                            existing_element.parent.name, existing_element.name, value
                        )
                    # else:
                    ...
                    if "def" in elem.type and "self" in value:
                        ...
                        self.replace_method(
                            existing_class.name, new_element.name, value
                        )
                    else:
...
class PyClass:
    ...
    def replace_method(self, new_element, existing_element, value):
        ...
        transformer = AddOrReplaceFunctionInClass(
            self.class_name, new_element.name, value
        )
        new_tree = transformer.visit(tree)
        new_tree = fix_missing_locations(new_tree)
        new_source = autopep8.fix_code(astor.to_source(new_tree))
