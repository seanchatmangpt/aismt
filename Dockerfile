...
ENV VIRTUAL_ENV /opt/aismt-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/aismt/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/aismt/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/aismt/ && touch src/aismt/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/aismt/
...
ENTRYPOINT ["/opt/aismt-env/bin/poe"]
CMD ["app"]
